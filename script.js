...
// Game settings
  ...
  settings.walls = true; // plate cannot go out of the screen
  settings.automatic = false; // the plate will not move by itself
  settings.godmode = false; // allows developer to access any point of the game
  settings.id = 0;
  settings.score = 0;
  ...
  // The player
  assets[0] = new Plate(settings, background);

  var Score = {
    lettuce: 10,  // Score = key : value
    meat: 15,
    tomato: 20,
    cheese: 5,
    topBun: 50,
...
// (which is why they have event listeners and are set to true)
  ...
  var ingredients = [
    {key: 'lettuce', speed: 5, isControlled: true},
    {key: 'tomato', speed: 8, isControlled: true},
    {key: 'cheese', speed: 6, isControlled: true},
    {key: 'meat', speed: 7, isControlled: true},
    {key: 'topBun', speed: 12},
    {key: 'chilliBomb', speed: 12}
  ];
...
//function to spawn food pushes them into the assets array.
  function spawnItem() {
    ...
    var ingredient = ingredients[random % 6];

    assets.push(new Ingredient(background, settings, ingredient.key, ingredient.speed, ingredient.isControlled));
  }
...
// Visibility function, passing clearAll as the argument 
  var visibility = function (clearAll) {
    ...
    assets = assets.filter(function (x) {
...
// The render function. It will be called 60f/sec
  this.render = function () {
    ...
    var onStack = assets.filter(function (item) {
      return item.stacked;
    });

    // Creates an array of items that are NOT-on-the-stack
    var offStack = assets.filter(function (item) {
      return !item.stacked;
    });
